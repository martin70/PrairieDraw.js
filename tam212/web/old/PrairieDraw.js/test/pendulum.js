...
Pendulum.prototype = new prairieDraw.Animator;
...
Pendulum.prototype.draw = function(t) {
    ...
    prairieDraw.selectColor(ctx, "black");
    ctx.lineWidth = 2;
    ctx.lineCap = "round";
    ...
    prairieDraw.selectColor(ctx, "white");
    ctx.fill();
    prairieDraw.selectColor(ctx, "black");
    ctx.stroke();
    ctx.restore();
    ...
    // angular momentum
    if (this.angMom) {
        prairieDraw.selectColor(ctx, "angMom");
        prairieDraw.circleArrow(ctx, 0, 0, angMomRadius, 3 * Math.PI / 2, 3 * omega);
    }
    ...
    if (this.velocity) {
        ...
        prairieDraw.selectColor(ctx, "velocity");
        prairieDraw.lineArrow(ctx, x, y, 0.3 * vx, 0.3 * vy);
    }
    ...
    if (this.acceleration) {
        ...
        prairieDraw.selectColor(ctx, "acceleration");
        prairieDraw.lineArrow(ctx, x, y, 0.3 * ax, 0.3 * ay);
    }

    // force
    if (this.force) {
        prairieDraw.selectColor(ctx, "force");
        prairieDraw.lineArrow(ctx, x, y, 0, -mgLength);
    }
